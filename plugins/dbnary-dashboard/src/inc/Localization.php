...
use GillesSerasset\Utils\Localization as UtilsLocalization;
...
class Localization {
    ...
    protected function getPackageInfo($type) {
        if ($type === UtilsLocalization::$PACKAGE_INFO_BACKEND) {
            return [path_join(DBNARY_DASHBOARD_PATH, 'languages'), DBNARY_DASHBOARD_TD];
        } else {
            return [path_join(DBNARY_DASHBOARD_PATH, Assets::$PUBLIC_JSON_I18N), DBNARY_DASHBOARD_TD];
        }
    }
}
