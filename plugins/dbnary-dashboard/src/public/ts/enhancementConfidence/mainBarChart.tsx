...
import {
    doEnhancementConfidenceDataCube,
...
const MainBarChart: FC<MainBarChartProps> = ({ decorations, provider, ...rest }) => {
    ...
    let inputLabels = [
        { key: "f1Measure", color: "#fdff00" },
        { key: "precisionMeasure", color: "#3ab09e" },
        { key: "recallMeasure", color: "#ff4f00" }
    ];

    useEffect(() => {
        doEnhancementConfidenceDataCube().then(normalizeSparqlData).then(setData);
    }, []);
